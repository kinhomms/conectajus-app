...
export function ClientDetailsPanel({
  ...
  return (
    <section className="rounded-3xl border border-white/10 bg-[#111827] p-8 shadow-xl shadow-black/20">
      <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-start">
        <div className="flex items-center gap-5">
          ...
          <div>
            ...
            <p className="mt-2 text-slate-400">Cliente ativo na plataforma</p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={onCreateAgendaEvent} className="mt-8 rounded-2xl border border-amber-400/20 bg-amber-400/10 p-5">
        <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-start">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.2em] text-amber-300">Próximo passo</p>
            <h2 className="mt-2 text-xl font-bold text-white">Agendar tarefa do atendimento</h2>
            <p className="mt-2 text-sm leading-6 text-slate-300">
              Crie uma tarefa vinculada a este cliente para continuar o atendimento na agenda.
            </p>
          </div>
          <Link href={routes.agenda} className="w-fit rounded-2xl border border-white/10 px-4 py-2 text-sm font-black text-white hover:bg-white/10">
            Ver agenda
          </Link>
        </div>
        ...
        <div className="mt-5 grid gap-3 lg:grid-cols-[1fr_220px_160px]">
          <label className="grid gap-2 text-sm font-semibold text-slate-300">
            ...
            <input
              ...
              className="rounded-2xl border border-white/10 bg-[#0B0F19] px-4 py-3 text-sm text-white outline-none placeholder:text-slate-500 focus:border-amber-400"
          ...
          <label className="grid gap-2 text-sm font-semibold text-slate-300">
            ...
            <input
              ...
              className="rounded-2xl border border-white/10 bg-[#0B0F19] px-4 py-3 text-sm text-white outline-none focus:border-amber-400"
            />
          </label>
        ...
        <button
          ...
          className="mt-4 rounded-2xl bg-amber-400 px-5 py-3 text-sm font-black text-black hover:bg-amber-300 disabled:cursor-not-allowed disabled:opacity-60"
        >
...
function Info({ title, value }: { title: string; value: string }) {
  return (
    <div className="rounded-2xl bg-[#0B0F19] p-5">
      <p className="text-xs uppercase tracking-widest text-amber-400">{title}</p>
