    ...
    return (
      <main className="min-h-screen bg-[#F5F7FB] px-6 py-10 text-[#07182F]">
        <div className="mx-auto max-w-3xl rounded-[2rem] border border-slate-200 bg-white p-8 shadow-sm">
          <p className="text-xs font-black uppercase tracking-[0.2em] text-teal-600">Perfil público</p>
    ...
    <main className="min-h-screen bg-[#F5F7FB] px-6 py-10 text-[#07182F]">
      <div className="mx-auto max-w-5xl">
        <div className="mb-6 flex flex-wrap gap-3">
          ...
          <Link href={routes.triage} className="rounded-2xl bg-teal-600 px-4 py-2 text-sm font-black text-white transition hover:bg-teal-500">
            Iniciar triagem
          </Link>
        </div>
        ...
        <section className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-xl shadow-slate-900/10">
          <div className="bg-[#07182F] p-8 text-white md:p-10">
            <p className="text-xs font-black uppercase tracking-[0.25em] text-teal-300">Advogado ConectaJus</p>
            <div className="mt-6 flex flex-col gap-6 md:flex-row md:items-center">
              {profile.profile_photo_url ? (
                <Image
                  ...
                  width={144}
                  height={144}
                  className="h-36 w-36 rounded-full object-cover ring-4 ring-teal-400/40"
                />
              ) : (
                <div className="flex h-36 w-36 items-center justify-center rounded-full bg-teal-600 text-5xl font-black text-white">
              ...
              <div>
                ...
                {oab ? (
                  <span className="mt-4 inline-flex rounded-full border border-teal-400/40 bg-teal-500/10 px-4 py-2 text-sm font-black text-teal-200">
                    OAB {oab}
                  </span>
                ) : null}
              </div>
          ...
          <div className="grid gap-6 p-8 md:p-10 lg:grid-cols-[1fr_320px]">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.2em] text-teal-600">Apresentação</p>
